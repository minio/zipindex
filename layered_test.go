...
func TestLayeredIndex_FindInLayer(t *testing.T) {
	...
	file, found = l.FindInLayer("file1.txt", "layer2")
	if found {
		t.Error("file1.txt should not exist in layer2")
	}
	
	// Non-existent layer
	file, found = l.FindInLayer("file1.txt", "nonexistent")
	if found {
...
func TestLayeredIndex_ImmutableReturns(t *testing.T) {
	...
	files1 = append(files1, FileWithRef[string]{
	...
	single1 = append(single1, File{Name: "injected2.txt"})
